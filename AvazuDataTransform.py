...
class AvazuDataTransform:
    ...
    def transform(self, train):
        ...
        if train:
            ...
            readFile = open(self.path + 'train.csv', "rt")
            delimiter1=','
        else:
            print('Transforming test data...')
            readFile = open(self.path + 'test.csv', "rt")
            delimiter1=';'
            diff = -1

        reader = csv.reader(readFile, delimiter=delimiter1)
        header = next(reader)
        ...
        if train:
            ...
            writers = []
            for i in range(10):
                writeFile = open(LR_path + "Part%d.csv" % i, "w")
                writers.append(csv.writer(writeFile))
                writers[i].writerow(header)
        else:
            writeFile = open(LR_path + "test.csv", "w")
    ...
    def update_rare_features(self, min_bound = 0):
        ...
        for i in range(11):
            if i == 10:
                ...
                readFile = open(self.path + "test.csv", "rt")
                writeFile = open(self.path + "test_none.csv", "wt")
                diff = -1
            else:
                print('Updating rare features for train data Part%d' % i)
                readFile = open(self.path + "Part%d.csv" % i, "rt")
                writeFile = open(self.path + "Part%d_none.csv" % i, "wt")
